...
function showApp() {
    ...
    try {
        ...
        const errorMsg = (typeof L === 'undefined') ? 
            "Leaflet (la libreria mappe) non è stata caricata correttamente." : 
...
// Popola la lista dei terremoti nel dropdown
function populateEarthquakeList(terremoti) {
    ...
    sortedTerremoti.forEach(terremoto => {
        ...
        listHTML += `
            <li class="list-row hover:bg-base-200 cursor-pointer p-2" data-lat="${lat}" data-lng="${lng}" data-mag="${mag}">
    ...
    earthquakeList.querySelectorAll('li').forEach(li => {
        ...
        const centerOnEarthquake = () => {
            ...
            if (earthquakeListDropdown) earthquakeListDropdown.removeAttribute('open');
        };

        li.addEventListener('click', (e) => {
            if (!e.target.closest('button')) centerOnEarthquake();
        });
...
function filterEarthquakes(minMag) {
    ...
    earthquakeMarkers.forEach(item => {
        ...
        if (item.mag >= minMag) terremotiLayer.addLayer(item.layer);
    });
}
...
async function initMapAndFetch() {
    if (typeof L === 'undefined') throw new Error("Leaflet non è definito.");
    ...
    geographicTile = L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png', {
    ...
    function switchBaseLayer(layerType) {
        ...
        layerMenu.querySelectorAll('a').forEach(a => a.classList.remove('font-bold'));
    ...
    try {
        ...
        for (const terremoto of terremoti) {
            ...
            if (raggioStimatoMetri !== null) {
                ...
                if (mag >= currentMinMag) terremotiLayer.addLayer(circle);
